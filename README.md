...
1. Sends API request to webserver 
  * Notifiy user if request was unsuccessful w/ failure reason
2. Package dependencies checked if they are already installed 
  * If dependency is installed, check VersionFile if version matching or greater than version in db
    * If less than version stored in db perform dependancy update command // check with user(advise suggesting it be completed)
  * Dependency is installed if it is missing
3. Check if package is already installed 
  * Check VersionFile if Version is latest
    * Perform update if not // Check with user first
